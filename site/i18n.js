...
const I18N = {
  vi: {
    "nav.faq": "Hỏi đáp",
    "faq.title": "Hỏi đáp",
    "q1.t": "macOS báo không kiểm tra được app, hoặc app có thể là malware.",
    "q1.p": "Bản build chưa được notarize nên macOS chặn lần mở đầu. Đóng hộp thoại đó, mở System Settings → Privacy & Security, kéo xuống chỗ “Deskfisch was blocked” và bấm Open Anyway. Trên macOS cũ hơn thì chuột phải vào app, chọn Open. Nếu vẫn không được, chạy lệnh này một lần trong Terminal rồi thử lại:",
    ...
    "q3.p": "Không. App không có tài khoản, không analytics, không báo cáo crash. App desktop chỉ có một loại kết nối mạng: kiểm tra bản mới trên GitHub Releases. Trang web đọc danh sách phiên bản từ GitHub API và đếm lượt xem bằng Cloudflare Web Analytics, công cụ này không dùng cookie và không theo dõi từng người truy cập.",
  ...
  es: {
    "nav.faq": "Preguntas",
    "faq.title": "Preguntas",
    "q1.t": "macOS dice que no pudo verificar la app o que podría ser malware.",
    "q1.p": "La compilación aún no está notarizada, así que macOS bloquea el primer inicio. Cierra ese aviso, abre Ajustes del Sistema → Privacidad y seguridad, baja hasta “Deskfisch was blocked” y pulsa Abrir de todos modos. En macOS más antiguos, haz clic derecho en la app y elige Abrir. Si sigue sin abrir, ejecuta esto una vez en Terminal y vuelve a intentarlo:",
    ...
    "q3.t": "¿Recopila algo?",
    "q3.p": "No. La app no tiene cuenta, ni analítica, ni informes de fallos. La app de escritorio hace un solo tipo de petición de red: comprobar actualizaciones en GitHub Releases. El sitio web lee la lista de versiones de la API de GitHub y cuenta las visitas con Cloudflare Web Analytics, que no usa cookies ni rastrea a cada visitante.",
  ...
  zh: {
    "nav.faq": "常见问题",
    "faq.title": "常见问题",
    "q1.t": "macOS 提示无法验证该应用，或提示它可能是恶意软件。",
    "q1.p": "构建尚未经过公证，所以 macOS 会拦截首次启动。关掉那个提示，打开“系统设置 → 隐私与安全性”，向下找到“Deskfisch 已被阻止”，点“仍要打开”。旧版 macOS 可以右键点击应用选择“打开”。仍然不行的话，在终端运行一次下面这行命令再试：",
    "q2.t": "Windows 弹出蓝色的 SmartScreen 窗口。",
    "q2.p": "点“更多信息”，再点“仍要运行”。安装包尚未签名，所以 Windows 没有发布者可显示。下载的文件正是 GitHub Actions 从公开源码构建的。",
    "q3.t": "它会收集什么吗？",
    "q3.p": "不会。应用没有账号，没有分析统计，没有崩溃上报。桌面应用只有一种网络请求：到 GitHub Releases 检查更新。网站从 GitHub API 读取版本列表，并用 Cloudflare Web Analytics 统计访问量；它不使用 Cookie，也不追踪单个访客。",
    "q4.t": "我的鱼缸存在哪里？",
  ...
  ko: {
    "nav.faq": "자주 묻는 질문",
    "faq.title": "자주 묻는 질문",
    "q1.t": "macOS가 앱을 확인할 수 없다거나 악성 소프트웨어일 수 있다고 표시합니다.",
    "q1.p": "아직 공증되지 않은 빌드라 macOS가 첫 실행을 막습니다. 그 창을 닫고 시스템 설정 → 개인정보 보호 및 보안으로 가서 아래쪽 “Deskfisch was blocked”에서 그래도 열기를 누르세요. 이전 macOS에서는 앱을 오른쪽 클릭해 열기를 고르면 됩니다. 그래도 안 되면 터미널에서 아래 명령을 한 번 실행하고 다시 시도하세요:",
    ...
    "q3.t": "무언가 수집하나요?",
    "q3.p": "아니요. 앱에는 계정도, 분석도, 오류 보고도 없습니다. 데스크톱 앱의 네트워크 요청은 GitHub Releases 업데이트 확인 한 가지뿐입니다. 웹사이트는 GitHub API에서 릴리스 목록을 읽고 Cloudflare Web Analytics로 방문 수를 셉니다. 쿠키를 쓰지 않고 방문자 개개인을 추적하지 않습니다.",
  ...
  ja: {
    "nav.faq": "よくある質問",
    "faq.title": "よくある質問",
    "q1.t": "macOS が「検証できない」または「マルウェアの可能性がある」と表示する。",
    "q1.p": "まだ公証（notarize）していないビルドなので、macOS が初回起動をブロックします。その表示を閉じて、システム設定 → プライバシーとセキュリティ を開き、下の方にある「Deskfisch はブロックされました」で「このまま開く」を押してください。古い macOS ではアプリを右クリックして「開く」を選びます。それでも開けない場合は、ターミナルで次のコマンドを一度実行してから再試行してください:",
    ...
    "q3.t": "何か収集していますか？",
    "q3.p": "いいえ。アプリにはアカウントも解析もクラッシュ報告もありません。デスクトップ版の通信は GitHub Releases への更新確認だけです。サイトは GitHub API からリリース一覧を取得し、Cloudflare Web Analytics で訪問数を数えています。Cookie は使わず、個々の訪問者を追跡しません。",
    "q4.t": "水槽はどこに保存されますか？",
